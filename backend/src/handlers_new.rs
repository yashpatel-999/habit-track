...
pub async fn update_habit(pool: web::Data<PgPool>, req: HttpRequest, path: web::Path<i32>, habit_data: web::Json<UpdateHabit>) -> Result<HttpResponse> {
    let auth_header = req.headers().get("Authorization")
        .and_then(|h| h.to_str().ok())
        .ok_or_else(|| actix_web::error::ErrorUnauthorized("Missing authorization header"))?;
...
pub async fn delete_habit(pool: web::Data<PgPool>, req: HttpRequest, path: web::Path<i32>) -> Result<HttpResponse> {
    let auth_header = req.headers().get("Authorization")
        .and_then(|h| h.to_str().ok())
        .ok_or_else(|| actix_web::error::ErrorUnauthorized("Missing authorization header"))?;
...
pub async fn log_habit(pool: web::Data<PgPool>, req: HttpRequest, path: web::Path<i32>, log_data: web::Json<CreateHabitLog>) -> Result<HttpResponse> {
    let auth_header = req.headers().get("Authorization")
        .and_then(|h| h.to_str().ok())
        .ok_or_else(|| actix_web::error::ErrorUnauthorized("Missing authorization header"))?;
...
pub async fn get_habit_progress(pool: web::Data<PgPool>, req: HttpRequest, path: web::Path<i32>) -> Result<HttpResponse> {
    let auth_header = req.headers().get("Authorization")
        .and_then(|h| h.to_str().ok())
        .ok_or_else(|| actix_web::error::ErrorUnauthorized("Missing authorization header"))?;
